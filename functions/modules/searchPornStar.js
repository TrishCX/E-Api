...
import got from "got";
export default async function searchStar(name) {
    ...
    if (isError !== null)
        throw Error("No PornStar found for the given name.");
...
Solo:")`).text();
    ...
    if (falseOrTrue === true) {
        ...
        const __solo = subStringedText.slice(subStringedText.lastIndexOf("Solo:") + 5, subStringedText.length);
        const solo = __solo.substring(0, __solo.lastIndexOf("Lesbian:"));
        const __lesbian = __solo.slice(__solo.lastIndexOf("Lesbian:") + 8, __solo.length);
        ...
        const girl_Girl = _girl_Girl
            ...
            .trim();
        const _boyGirl = _girl_Girl.slice(_girl_Girl.lastIndexOf("Boy/girl:") + 9, _girl_Girl.length);
